...
from PyQt5.QtWidgets import QPushButton, QListWidget, QAbstractItemView, QListWidgetItem, QMessageBox
...
class MergeFileList(QListWidget):
    """
    Grid de miniaturas (primera página) de PDFs para la pestaña Unir PDFs.
    - Acepta drag & drop de archivos PDF desde el explorador.
    - Reordenación interna con resaltado del destino, igual que PageThumbnailList.
    - Cada item guarda la ruta completa en Qt.UserRole.
    """
    ...
    def add_pdf(self, path: str):
        """Añade un PDF renderizando la miniatura de su primera página."""
        from core.pdf_reorder import render_icon
        try:
            icon  = render_icon(path, 0)
            fname = os.path.basename(path)
            item  = QListWidgetItem(icon, fname)
            item.setTextAlignment(Qt.AlignHCenter | Qt.AlignBottom)
            item.setSizeHint(QSize(130, 195))
            item.setData(Qt.UserRole, path)
            item.setToolTip(path)
            self.addItem(item)
        except Exception as e:
            QMessageBox.critical(None, 'Error al cargar PDF', str(e))
    ...
    def dropEvent(self, event):
        ...
        if event.mimeData().hasUrls():
            ...
            for url in event.mimeData().urls():
                if url.isLocalFile() and url.toString().lower().endswith('.pdf'):
                    self.add_pdf(str(url.toLocalFile()))
